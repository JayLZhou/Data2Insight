# -*- coding: utf-8 -*-
from core.agents import GudingAgent, InsightAgnet, Refiner,DataAgent, MemoryAgent
from core.const import MAX_ROUND, SYSTEM_NAME, GUDING_NAME, REFINER_NAME, INSIGHT_NAME
...
class ChatManager(object):
    def __init__(self, data_path: str, tables_json_path: str, log_path: str, model_name: str, dataset_name:str, lazy: bool=False, without_selector: bool=False):
        ...
        self.ping_network()
        self.chat_group = [
            GudingAgent(data_path=self.data_path, tables_json_path=self.tables_json_path, model_name=self.model_name, dataset_name=dataset_name, lazy=lazy, without_selector=without_selector),
            InsightAgnet(dataset_name=dataset_name),
            Refiner(data_path=self.data_path, dataset_name=dataset_name),
            DataAgent(data_path=self.data_path, dataset_name=dataset_name),
            MemoryAgent(data_path=self.data_path, dataset_name=dataset_name)
        ]
