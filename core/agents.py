# -*- coding: utf-8 -*-
from core.utils import parse_json, parse_qa_pairs, parse_sql_from_string, add_prefix, load_json_file, extract_world_info, is_email, is_valid_date_column, parse_single_sql, distance
from random import sample

# import insightenvs, wrappers
LLM_API_FUC = None
...
class GudingAgent(BaseAgent):
    """
    Act as a data scientist to plan for the goal
    """
    name = GUDING_NAME
    description = "Act as a data scientist to plan for the goal"
    ...
    def step(self, message: dict):
        ...
        db_id, ext_sch, query, insights = message.get('db_id'), \
                                          message.get('extracted_schema', {}), \
                                          message.get('query'), \
                                          message.get('memory_insights')
        if query is None:
            query = self.goal
        else:
            self.goal = query
        ...
        if 'memory_insights' in message:
            del message['memory_insights'] # no need for other agents.
        message['guding_dict'] = extracted_schema_dict
        message['chosen_db_schem_dict'] = chosen_db_schem_dict
        message['desc_str'] = db_schema
        message['fk_str'] = db_fk
        message['send_to'] = INSIGHT_NAME



class InsightAgnet(BaseAgent):
    """
    Decompose the question and solve them using CoT
    """
    name = INSIGHT_NAME
    description = "Mining insights"

    def __init__(self, dataset_name):
        super().__init__()
        self.dataset_name = dataset_name
        self._message = {}
        # self.env = insightenv.InsightEnv()
    ...
    def step(self, message: dict):
        ...
        if message.get('table') == None:
            message['send_to'] = DATA_NAME
            return
        guiding_dict = message.get('guding_dict')
        task = guiding_dict['task']
        interesting_columns = guiding_dict['interesting_columns']
        table = message.get('table')
        analysis_trajs = message.get('analysis_trajs')    
        if analysis_trajs == None: message['analysis_trajs'] = []
        prompt = insight_template.format(desc_str=schema_info,task=task, table = table, analysis_trajs = analysis_trajs, relevant_column = interesting_columns)
        if analysis_trajs != None and len(analysis_trajs) > 5: 
            prompt += "Please summarize the insights based on the analysis history and input table. \n"
        ...
        while bad_access < 4 and not early_stpo:
            ...
            if len(etracted_insight_dict['analysis_output']) != 2:
                ...
                early_stpo = True
        if etracted_insight_dict['insight'] != "":
            # clear the message
            if 'prefer_data' in message:
                del message['prefer_data']
            if 'table' in message:
                del message['table']
            if 'analysis_trajs' in message:
              del message['analysis_trajs']
            message['insight'] = etracted_insight_dict['insight']
            message['send_to'] = MEMORY_NAME
        else:
            message['prefer_data'] = etracted_insight_dict['prefer_data']
            message['interesting_columns'] = etracted_insight_dict['interesting_columns']
            message['analysis_trajs'].append(etracted_insight_dict['analysis_output'])
            message['send_to'] = DATA_NAME
...
class DataAgent(BaseAgent):
    ...
    def step(self, message: dict):
        ...
        if message['send_to'] != self.name: return
        self._message = message
        db_id, old_sql, schema_info = message.get('db_id'), \
                                     message.get('pred', message.get('final_sql')), \
                                            message.get('desc_str')
        guiding_dict = message.get('guding_dict')
        task = guiding_dict['task']
        interesting_columns = guiding_dict['interesting_columns']     
        prefer_data = message.get('prefer_data') 
        analysis_trajs = message.get('analysis_trajs')                                          
        prompt = data_template.format(desc_str=schema_info,task=task, relevant_column = interesting_columns, sql = old_sql, prefer_data = prefer_data, analysis_trajs = analysis_trajs)
...
class MemoryAgent(BaseAgent):
    ...
    name = MEMORY_NAME
    description = "Store insights and summary insights in memory"
    ...
    def step(self, message: dict):
        ...
        if message['send_to'] != self.name: return
        self._message = message
        insight, task = message.get('insight'), message.get('task')    
        import pdb
        pdb.set_trace()
        print(insight)
        print("--------------------------------")
        print(self._insights_memory)
        prompt = memory_template.format(insight=insight, insight_memory = self._insights_memory)

        reply = LLM_API_FUC(prompt).strip()
        insight_memory = parse_json(reply)
        self._insights_memory = insight_memory['insight_memory']
        print("after----------------------------")
        print(self._insights_memory)
        # import pdb
        # pdb.set_trace()
        message['send_to'] = GUDING_NAME
        message['memory_insights'] = self._insights_memory
...
class Refiner(BaseAgent):
    ...
    def __init__(self, data_path: str, dataset_name: str):
        ...
        self.dataset_name = dataset_name
        self._message = {}
    def _sample_tuples(self, table, b = 300):
        """
        Sample tuples from the bit table
        :param message: table: get the table via generated sql.
        "return: return b tuples from the table.
        """
  
        if len(table) * len(table[0]) <= 1200 or len(table) <= b:
            return table
        else:
            return sample(table, b)
            # select_tuple = [choice(table)]
            # table.remove(select_tuple[0])
            # select_tuple_set = set(select_tuple)  # Convert list to set for faster lookups
            # table_set = set(table)  # Assuming table can be converted to a set

            # while len(select_tuple_set) < b and table_set:
            #     max_distance = 0
            #     min_u = None  # Initialize to None to handle the case where no element is found
                
            #     for x_i in table_set:
            #         min_distance = float('inf')
                    
            #         # Vectorize this part if possible, or use a more efficient loop if not
            #         for x_j in select_tuple_set:    
            #             if x_i != x_j:
            #                 dis_ = distance(x_i, x_j)
            ...
            #     if min_u and min_u not in select_tuple_set:           
            #         select_tuple_set.add(min_u)
            #         table_set.remove(min_u)

            # # Convert select_tuple_set back to a list if necessary
            # select_tuple = list(select_tuple_set)
            return select_tuple
    def _execute_sql(self, sql: str, db_id: str) -> dict:
        ...
        try:
            ...
            return_data = [str(column_names)[1:-1]] + self._sample_tuples(result)
            return {
                "sql": str(sql),
                "data": return_data,
                "sqlite_error": "",
