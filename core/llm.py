...
def api_func(prompt:str, stop = None):
    global MODEL_NAME
    print(f"\nUse OpenAI model: {MODEL_NAME}\n")
    if MODEL_NAME.startswith('CodeLlama'):
        client.api_base = 'http://0.0.0.0:8000/v1'
    import pdb
...
def safe_call_llm(input_prompt, stop = None) -> str:
    """
    ...
    for i in range(5):
        try:
            if log_path is None:
                # print(input_prompt)
                sys_response, prompt_token, response_token = api_func(input_prompt, stop)
                print(f"\nsys_response: \n{sys_response}")
